...
def SaveIonMoments(Date,sc):	
	...
	if not uhr is None:
		...
		no_ne = True
	if not no_ne:
		fne = interp1d(utcne,ne,fill_value='extrapolate',bounds_error=False)
	...
	print('Hydrogen')
	nH,TH,pH = IntegrateSpectra(sH.Energy[0],sH.E0[0],sH.E1[0],sH.Spec[0],_ECT.mass['H'],4*np.pi,out.Vsc,out.vbulk,Erange=(0.0,0.02))
	print('Helium')
	nHe,THe,pHe = IntegrateSpectra(sHe.Energy[0],sHe.E0[0],sHe.E1[0],sHe.Spec[0],_ECT.mass['He'],4*np.pi,out.Vsc,out.vbulk,Erange=(0.0,0.02))
	print('Oxygen')
	nO,TO,pO = IntegrateSpectra(sO.Energy[0],sO.E0[0],sO.E1[0],sO.Spec[0],_ECT.mass['O'],4*np.pi,out.Vsc,out.vbulk,Erange=(0.0,0.02))
	...
	out.H_T_c_ul[:,0] = np.float64((np.sqrt(2)*dEH)/(erfinv(1.0 - (nH/out.ne))))/kB
	out.He_T_c_ul[:,0] = np.float64((np.sqrt(2)*dEHe)/(erfinv(1.0 - (nHe/(0.15*out.ne)))))/kB
	out.O_T_c_ul[:,0] = np.float64((np.sqrt(2)*dEO)/(erfinv(1.0 - (nO/(0.01*out.ne)))))/kB
