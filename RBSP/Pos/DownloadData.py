...
from ._GetH5URL import _GetH5URL
import os
import re
...
def DownloadData(sc='a',StartYear=2012,EndYear=2019,Overwrite=False):
	'''
	Downloads magnetice ephemeris data.
	...
	outpath = Globals.DataPath+'MagEph/{:s}/'.format(sc)
	...
	for i in range(0,n):
		...
		urls,fnames = _GetH5URL(Years[i],sc)
		nu = np.size(urls)
		...
		for j in range(0,nu):
			...
			if (not match) or Overwrite:
				if not os.path.isfile(outpath+fnames[j]) or Overwrite:
					os.system('wget --no-check-certificate -nv '+urls[j]+' -O '+outpath+fnames[j])
