"""
This code originates from http://www.jtrive.com/loess-nonparametric-scatterplot-smoothing-in-python.html
I couldn't find a package which contains this function.
...
def loess(xvals, yvals, alpha, poly_degree=1, robustify=False):
    ...
    for i in v:
        ...
        iterpos = i[0]
        iterval = i[1]
        print('\r{0} of {1}'.format(i[0]+1,m),end='')
        ...
        locsDF = pd.concat([locsDF, iterDF1],sort=True)
        W = np.diag(weights)
        y = yvals
        ...
        evalDF = pd.concat([evalDF, iterDF2],sort=True)
    print()
